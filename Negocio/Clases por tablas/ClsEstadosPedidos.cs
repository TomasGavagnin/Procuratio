...
namespace Negocio
{
    public class ClsEstadosPedidos : EstadoPedido
    {
        public enum EEstadosPedidos
        {
            Pendiente = 1, EnProceso, ParaEntrega, Entregado, EsperandoPago, Eliminado, Finalizado
        }
        ...
        public List<EstadoPedido> LeerListado(ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoPedido.Include("?????????").ToList();
                }
        ...
        /// </summary>
        /// <param name="_ID_EstadoPedidoBuscar">ID del registro que se desea traer su informacion.</param>
        ///<param name="_InformacionDelError">Devuelve una cadena de texto con informacion para el usuario en caso de que el
        /// metodo devuelva null (debido a que ocurrio un error).</param>
        public EstadoPedido LeerPorNumero(int _ID_EstadoPedidoBuscar, ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoPedido.SingleOrDefault(Identificador => Identificador.ID_EstadoPedido == _ID_EstadoPedidoBuscar);
                }
                catch (Exception Error)
                {
                    _InformacionDelError = $"OCURRIO UN ERROR INESPERADO AL INTENTAR LISTAR LA INFORMACIÓN: {Error.Message}\r\n\r\n" +
        ...
        /// </summary>
        /// <param name="_EstadoPedido">Objeto que contiene los datos del nuevo registro que se creará.</param>
        ///<param name="_InformacionDelError">Devuelve una cadena de texto con informacion para el usuario en caso de que el
        /// metodo devuelva null (debido a que ocurrio un error).</param>
        public int Crear(EstadoPedido _EstadoPedido, ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    BBDD.EstadoPedido.Add(_EstadoPedido);
