...
namespace Negocio
{
    public class ClsEstadosCajas : EstadoCaja
    {
        public enum EEstadosCajas
        {
            Activo = 1, Inactivo, CajaAbierta
        }
        ...
        public List<EstadoCaja> LeerListado(ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoCaja.Include("?????????").ToList();
                }
        ...
        /// </summary>
        /// <param name="_ID_EstadoCajaBuscar">ID del registro que se desea traer su informacion.</param>
        ///<param name="_InformacionDelError">Devuelve una cadena de texto con informacion para el usuario en caso de que el
        /// metodo devuelva null (debido a que ocurrio un error).</param>
        public EstadoCaja LeerPorNumero(int _ID_EstadoCajaBuscar, ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoCaja.SingleOrDefault(Identificador => Identificador.ID_EstadoCaja == _ID_EstadoCajaBuscar);
                }
                catch (Exception Error)
                {
                    _InformacionDelError = $"OCURRIO UN ERROR INESPERADO AL INTENTAR LISTAR LA INFORMACIÓN: {Error.Message}\r\n\r\n" +
        ...
        /// </summary>
        /// <param name="_EstadoCaja">Objeto que contiene los datos del nuevo registro que se creará.</param>
        ///<param name="_InformacionDelError">Devuelve una cadena de texto con informacion para el usuario en caso de que el
        /// metodo devuelva null (debido a que ocurrio un error).</param>
        public int Crear(EstadoCaja _EstadoCaja, ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    BBDD.EstadoCaja.Add(_EstadoCaja);
        ...
        ///<param name="_InformacionDelError">Devuelve una cadena de texto con informacion para el usuario en caso de que el
        /// metodo devuelva null (debido a que ocurrio un error).</param>
        public int Actualizar(EstadoCaja _EstadoCaja, ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    EstadoCaja ObjetoActualizado = BBDD.EstadoCaja.SingleOrDefault(Identificador => Identificador.ID_EstadoCaja == _EstadoCaja.ID_EstadoCaja);

                    if (ObjetoActualizado != null)
                    {
                        ObjetoActualizado.ID_EstadoCaja = _EstadoCaja.ID_EstadoCaja;
        ...
        /// </summary>
        /// <param name="_ID_EstadoCajaEliminar">Registro que se eliminará.</param>
        /// <param name="_InformacionDelError">Devuelve una cadena de texto con informacion para el usuario en caso de que el
        /// metodo devuelva null (debido a que ocurrio un error).</param>
        public int Borrar(int _ID_EstadoCajaEliminar, ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    EstadoCaja ObjetoAEliminar = BBDD.EstadoCaja.SingleOrDefault(Identificador => Identificador.ID_EstadoCaja == _ID_EstadoCajaEliminar);

                    if (ObjetoAEliminar != null)
                    {
                        BBDD.EstadoCaja.Remove(ObjetoAEliminar);
                        return BBDD.SaveChanges();
                    }
                    else
                    {
