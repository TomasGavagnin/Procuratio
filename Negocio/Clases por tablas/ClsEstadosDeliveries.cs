...
{
    public class ClsEstadosDeliveries : EstadoDelivery
    {
        public enum EEstadosDeliveries
        {
            Entregado = 1, ParaEntrega, Eliminado, NoEntregado, EnCocina
        }
        ...
        public List<EstadoDelivery> LeerListado(ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoDelivery.ToList();
                }
        ...
        /// </summary>
        /// <param name="_ID_EstadoDeliveryBuscar">ID del registro que se desea traer su informacion.</param>
        ///<param name="_InformacionDelError">Devuelve una cadena de texto con informacion para el usuario en caso de que el
        /// metodo devuelva null (debido a que ocurrio un error).</param>
        public EstadoDelivery LeerPorNumero(int _ID_EstadoDeliveryBuscar, ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoDelivery.SingleOrDefault(Identificador => Identificador.ID_EstadoDelivery == _ID_EstadoDeliveryBuscar);
                }
                catch (Exception Error)
                {
                    _InformacionDelError = $"OCURRIO UN ERROR INESPERADO AL INTENTAR LISTAR LA INFORMACIÓN: {Error.Message}\r\n\r\n" +
