...
namespace Negocio
{
    public class ClsEstadosArticulos : EstadoArticulo
    {
        public enum EEstadosArticulos
        {
            Activo = 1, Inactivo
        }
        ...
        public List<EstadoArticulo> LeerListado(ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoArticulo.Include("?????????").ToList();
                }
        ...
        ///<param name="_InformacionDelError">Devuelve una cadena de texto con informacion para el usuario en caso de que el
        /// metodo devuelva null (debido a que ocurrio un error).</param>
        public EstadoArticulo LeerPorNumero(int _ID_EstadoArticuloBuscar, ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoArticulo.SingleOrDefault(Identificador => Identificador.ID_EstadoArticulo == _ID_EstadoArticuloBuscar);
                }
                catch (Exception Error)
                {
                    _InformacionDelError = $"OCURRIO UN ERROR INESPERADO AL INTENTAR LISTAR LA INFORMACIÓN: {Error.Message}\r\n\r\n" +
