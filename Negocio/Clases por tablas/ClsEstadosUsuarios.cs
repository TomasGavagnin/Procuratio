...
namespace Negocio
{
    public class ClsEstadosUsuarios : EstadoUsuario
    {
        ...
        public List<EstadoUsuario> LeerListado(ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoUsuario.Include("?????????").ToList();
                }
        ...
        /// </summary>
        /// <param name="_ID_EstadoUsuarioBuscar">ID del registro que se desea traer su informacion.</param>
        ///<param name="_InformacionDelError">Devuelve una cadena de texto con informacion para el usuario en caso de que el
        /// metodo devuelva null (debido a que ocurrio un error).</param>
        public EstadoUsuario LeerPorNumero(int _ID_EstadoUsuarioBuscar, ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoUsuario.SingleOrDefault(Identificador => Identificador.ID_EstadoUsuario == _ID_EstadoUsuarioBuscar);
                }
                catch (Exception Error)
                {
                    _InformacionDelError = $"OCURRIO UN ERROR INESPERADO AL INTENTAR LISTAR LA INFORMACIÓN: {Error.Message}\r\n\r\n" +
