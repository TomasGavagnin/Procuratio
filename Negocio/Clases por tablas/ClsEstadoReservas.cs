...
namespace Negocio
{
    public class ClsEstadoReservas : EstadoReserva
    {
        public enum EEstadosReservas
        {
            Pendiente = 1, SinConfirmar, Eliminada, Completada, NoVino
        }
        ...
        public List<EstadoReserva> LeerListado(ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoReserva.ToList();
                }
        ...
        /// </summary>
        /// <param name="_ID_EstadoReservaBuscar">ID del registro que se desea traer su informacion.</param>
        ///<param name="_InformacionDelError">Devuelve una cadena de texto con informacion para el usuario en caso de que el
        /// metodo devuelva null (debido a que ocurrio un error).</param>
        public EstadoReserva LeerPorNumero(int _ID_EstadoReservaBuscar, ref string _InformacionDelError)
        {
            using (BDRestauranteEntities BBDD = new BDRestauranteEntities())
            {
                try
                {
                    return BBDD.EstadoReserva.SingleOrDefault(Identificador => Identificador.ID_EstadoReserva == _ID_EstadoReservaBuscar);
                }
                catch (Exception Error)
                {
                    _InformacionDelError = $"OCURRIO UN ERROR INESPERADO AL INTENTAR LISTAR LA INFORMACIÓN: {Error.Message}\r\n\r\n" +
